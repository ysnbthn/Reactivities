...
export default class ActivityStore{
    ...
    constructor(){
        ...
        // keylerden biri değişirse çalış
        reaction(
            () => this.predicate.keys(),
            ()=>{
    ...
    // date aynı kalacak fakat üstteki üç filtreden birini seçebilecek
    ...
    private setActivity = (activity: Activity) => {
        ...
        if(user){
            // arraydeki tek element bile koşulu sağlıyorsa true döner
    ...
    updateAttendance = async () =>{
        // etkinlikte yoksa ekle varsa çıkart
        const user = store.userStore.user;
        this.loading = true;
        try{
    ...
    cancelActivityToggle = async () => {
        ...
        try {
            await agent.Activities.attend(this.selectedActivity!.id);
            runInAction(()=>{
                // iptal edildiyse tersine çevir yoksa iptal et
        ...
        this.activityRegistry.forEach(activity =>{
            activity.attendees.forEach(attendee=>{
                if(attendee.username === username){
                    // follow counter güncelle 
                    attendee.following ? attendee.followersCount-- : attendee.followersCount ++;
                    // followsa unfollow yoksa follow yap
